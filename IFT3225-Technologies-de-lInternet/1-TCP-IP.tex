    ...
    \begin{center}
        \begin{tikzpicture}[dirtree]
            \node[directory] {\fontfamily{lmss}\selectfont Types de Requêtes HTTP}
                child { node[directory] {GET} 
                child {node  {\fontfamily{lmss}\selectfont Aucun corps, simplement pour accéder à une ressource}}
            }
            child[missing] {}
            child { node[directory] {POST}
                child {node  {\fontfamily{lmss}\selectfont Envoie un paramètre dans le corps de la requête}
            }}
            child[missing] {}
            child { node[directory] {PUT}
                child {node  {\fontfamily{lmss}\selectfont Modifie une ressource existante}
            }}
            child[missing] {}
            child { node[directory] {DELETE}
                child { node  {Supprime une ressource}}
            }
            child[missing] {}
            child[directory]{ node{HEAD}
                child { node {Uniquement pour l'envoie d'en-tête}}
            }
            ;
        \end{tikzpicture}%
    \end{center}
    ...
    curl https://www.google.ca/
        ...
        \begin{center}
            \hyperlink{Requête simple avec curl}{
                \xtargetbox[myb]{\fontfamily{lmss}\selectfont Détails de la requête \texttt{GET} simple\;}
            }
        \end{center}
    \end{EExample}
    ...
    curl -v https://www.google.ca/
        ...
        \begin{center}
            \hyperlink{Requête détaillée avec curl}{
                \xtargetbox[myb]{\fontfamily{lmss}\selectfont Détails de la requête \texttt{GET} détaillée   \;}
            }
        \end{center}
    \end{EExample}
    ...
    curl -o out -v https://diro.umontreal.ca/accueil/
        ...
        \begin{center}
            \hyperlink{Télécharger une page web avec curl}{
                \xtargetbox[myb]{\fontfamily{lmss}\selectfont Détails de la réponse\;}
            }
        \end{center}
    \end{EExample}
...
curl -v http://diro.umontreal.ca
    ...
    \begin{center}
        \hyperlink{Redirection temporaire avec curl}{
            \xtargetbox[myb]{\fontfamily{lmss}\selectfont Détails des réponses\;}
        }
    \end{center}
\end{EExample}
    ...
    % \chapter{Introduction au scriptage en \texttt{csh}}
    %
    % \begin{note}{}{}
    %     Le commande \texttt{iconv} permet de convertir les caractères   
    % \end{note}
    %
    % setenv LANG fr_CA
...
\noindent\xfbox[black!80!cyan!40]{\texttt{-v}} 
...
\noindent\xfbox[black!80!cyan!40]{\texttt{--url pop3://mail.iro.umontreal.ca}} 
...
\noindent\xfbox[black!80!cyan!40]{\texttt{--insecure}} 
    désactive la vérification des certificats SSL/TLS (à éviter en production);

\noindent\xfbox[black!80!cyan!40]{\texttt{--user}} 
...
\noindent\xfbox[black!80!cyan!40]{\hypertarget{Examiner une boîte de réception avec EXAMINE INBOX}{\texttt{--request}}} 
...
\noindent\xfbox[black!80!cyan!40]{\texttt{UID}} 
...
\xfbox[black!80!cyan!40]{\hypertarget{Télécharger une page web avec curl}{\texttt{Pour la réponse suivante}}}
\begin{verbatim}
...
\end{verbatim}
\noindent\xfbox[black!80!cyan!40]{\texttt{HTTP/1.1 301 Moved Permanently}} indique que la ressource a été 
déplacée de façon permanente.  

\noindent\xfbox[black!80!cyan!40]{\texttt{Date}} spécifie l’heure de la réponse : \texttt{Tue, 07 Jan 2020 03:40:45 GMT}.  

\noindent\xfbox[black!80!cyan!40]{\texttt{Server}} indique que le serveur utilisé est \texttt{Apache}.  

\noindent\xfbox[black!80!cyan!40]{\texttt{Location}} redirige vers l’URL \texttt{http://diro.umontreal.ca/}.  

\noindent\xfbox[black!80!cyan!40]{\texttt{Content-Length}} indique que la longueur du contenu est de 351 octets.  

\noindent\xfbox[black!80!cyan!40]{\texttt{Content-Type}} précise que le contenu est du HTML encodé 

\subsection{Détail des réponses \emph{Moved Permanently} et \emph{Temporary Redirect}}
\xfbox[black!80!cyan!40]{\hypertarget{Redirection temporaire avec curl}{\texttt{Pour les réponses suivantes}}}
\begin{verbatim}
HTTP/1.1 301 Moved Permanently
...
\noindent\xfbox[black!80!cyan!40]{\texttt{Location}} redirige vers une version HTTPS de l'URL.

\noindent\xfbox[black!80!cyan!40]{\texttt{Server}} précise que le serveur utilisé est \texttt{BigIP}.  

\noindent\xfbox[black!80!cyan!40]{\texttt{Connection}} spécifie que la connexion reste active (\texttt{Keep-Alive}).  

\noindent\xfbox[black!80!cyan!40]{\texttt{HTTP/1.1 307 Temporary Redirect}} indique une redirection temporaire 
vers \texttt{https://diro.umontreal.ca/accueil/}.  

\noindent\xfbox[black!80!cyan!40]{\texttt{Date}} précise l’heure de la réponse : \texttt{Mon, 06 Jan 2020 05:50:18 GMT}.  

\noindent\xfbox[black!80!cyan!40]{\texttt{Vary}} informe que la réponse varie selon l’hôte.  

\noindent\xfbox[black!80!cyan!40]{\texttt{Set-Cookie}} configure un cookie pour des sessions futures, 
